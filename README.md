...
A clean, modular .NET API for managing events, tickets, reservations, and purchases — built with EF Core, Clean Architecture, and full optimistic concurrency support.

📌 Overview

The EventTicketing API provides endpoints for:

Creating and retrieving events

Viewing event tickets

Reserving tickets (with concurrency protection)

Purchasing tickets

Ensuring safe updates using EF Core RowVersion
...
Ticket can be reserved

Reserved

Ticket is temporarily held
...
The API uses optimistic concurrency via EF Core:
...
/api/events/{eventId}/tickets/reserve
...
Concurrency tests (SQLite in-memory)
